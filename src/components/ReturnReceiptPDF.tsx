...
// components/ReturnReceiptPDF.js
...
interface ReturnReceiptPDFProps {
  lease: LeaseProps;
  devolutionData: DevolutionDataProps;
}
const styles = StyleSheet.create({
  page: {
    ...
    fontSize: 8, // Fonte reduzida para caber em meia página
  },
  header: {
    fontSize: 10,
    marginBottom: 8,
...
const ReturnReceiptPDF: React.FC<ReturnReceiptPDFProps> = ({
  ...
  const currentDate = new Date(devolutionData.dataDevolucao).toLocaleDateString(
    "pt-BR",
    {
      day: "2-digit",
      month: "2-digit",
    ...
    <Document>
      <Page size="A5" style={styles.page}>
        <View style={styles.header}>
          <Text>TERMO DE DEVOLUÇÃO DE EQUIPAMENTOS</Text>
        </View>
        <View
          style={{
            height: 50,
        ...
        <View style={styles.content}>
          <Text>
            Nós da <Text style={styles.bold}>LOCFRAN LTDA</Text>, inscrita no
            CNPJ sob o nº <Text style={styles.bold}>12.345.648/0001-90</Text>,
            com sede em [ENDEREÇO COMPLETO], certificamos que, nesta data,{" "}
            <Text style={styles.bold}>{currentDate}</Text>, o(a) Sr.(a){" "}
            <Text style={styles.bold}>{lease.cliente?.name}</Text>, portador(a)
            do CPF nº <Text style={styles.bold}>{lease.cliente?.cpf_cnpj}</Text>
            , devolveu integralmente os equipamentos objeto do Contrato de
            Locação nº <Text style={styles.bold}>{lease.id_locacao}</Text>,
          ...
          {lease.leaseItems.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              <Text style={styles.tableCell}>
                {item.patrimonio.produto.name} ({item.patrimonio.produto.marca})
              </Text>
              <Text style={styles.tableCell}>
                {item.patrimonio.numero_patrimonio}
              </Text>
              <Text style={styles.tableCell}>{item.patrimonio.status}</Text>
            </View>
          ))}
        </View>

        <View style={styles.declarations}>
          <Text style={[styles.content, styles.bold]}>Declarações:</Text>

          <View style={styles.declarationItem}>
            <Text>1. </Text>
            <Text>
              O(a) locatário(a) atesta que os equipamentos foram devolvidos em
              conformidade com o contrato, sem avarias além do desgaste normal
              de uso, salvo observações em contrário.
            </Text>
          </View>
          ...
          <View style={styles.declarationItem}>
            <Text>2. </Text>
            <Text>
              A locadora confirma o recebimento dos itens listados, isentando
              o(a) locatário(a) de quaisquer ônus futuros relacionados à locação
              em questão.
            </Text>
          </View>
        </View>
        ...
        <Text style={styles.content}>
          Este documento serve como recibo de quitação e encerra obrigações
          entre as partes relativas ao Contrato nº{" "}
          <Text style={styles.bold}>{lease.id_locacao}</Text>.
        </Text>

        <View style={styles.signatureSection}>
          <View style={styles.signatureLine}>
            <Text>Franca/SP, {currentDate}</Text>
            <Text>LOCADORA</Text>
          </View>
