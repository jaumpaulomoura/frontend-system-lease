...
const LeaseContractPDF = ({ lease }: { lease?: LeaseProps | null }) => {
  ...
  const groupedItems = lease.leaseItems?.reduce<Record<string, GroupedItem>>(
    (acc, item) => {
      ...
      // Calcula o valor correto do item
      ...
      if (item.valor_total != null && item.valor_total > 0) {
        ...
        const valorNegociado = parseToNumber(
          periodo === "diario"
        ...
        const dias = parseToNumber(item.quantidade_dias || 1);
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <Text style={{ fontWeight: "bold" }}>LOCAL DE ENTREGA/OBRA:{" "}
            {[
          ...
          <Text style={{ fontWeight: "bold" }}>
            TELEFONE: {lease.telefone_contato || lease.cliente?.telefone || "NÃO INFORMADO"}
          </Text>
          {/*  DO OBJETO */}
